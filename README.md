...
`Compute_f2_and_make_tree_heatmap_MDS.R` --> To compute f2-statistics from EIGENSTRAT file format with [admixtools](https://uqrmaie1.github.io/admixtools/index.html). Then make and save a simple NJ tree in newick format, a heatmap and the MDS. Population column of the .ind file of the EIGENSTRAT format needs to be changed accordingly (one sample = one population for individual pairwise analyses, like tree and interpolation; samples grouped by sector/time slices for comparisons between groups, like in the heatmap).
